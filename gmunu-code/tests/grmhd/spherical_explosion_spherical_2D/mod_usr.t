module mod_usr
  ...
  double precision, parameter            :: Y_0 = 0.0d0!-0.5d0
...
contains
  ...
  subroutine rm_init_one_grid(ixI^L,ixO^L,w,x)
    ...
    double precision, parameter     :: Bz = 0.1d0
    ...
    w(ixO^S,Bvec(1))    =   Bz * dcos( x(ixO^S,2) )
    w(ixO^S,Bvec(2))    = - Bz * dsin( x(ixO^S,2) ) / x(ixO^S,1)
    w(ixO^S,Bvec(3))    = 0.0d0

    where ( x(ixO^S,1) < r_in )
       w(ixO^S,rho_)       = rho_in
       w(ixO^S,press_)     = p_in
    else where ( x(ixO^S,1) > r_out )
       w(ixO^S,rho_)       = rho_out
       w(ixO^S,press_)     = p_out
    else where
       w(ixO^S,rho_)       = dexp( ( (r_out - x(ixO^S,1))*dlog(rho_in) + (x(ixO^S,1)-r_in)*dlog(rho_out) )/(r_out-r_in) )
       w(ixO^S,press_)     = dexp( ( (r_out - x(ixO^S,1))*dlog(p_in) + (x(ixO^S,1)-r_in)*dlog(p_out) )/(r_out-r_in) )
    end where

    w(ixO^S, divB_) = 1.0d-40

    w(ixO^S, B2_) = w(ixO^S, Bvec(1))**2 &
                  + x(ixO^S, 1)**2 * w(ixO^S, Bvec(2))**2 &
                  + x(ixO^S, 1)**2 * dsin(x(ixO^S, 2))**2 * w(ixO^S, Bvec(3))**2
    w(ixO^S, mag_press_) = w(ixO^S, Bvec(1))*w(ixO^S, W_vel(1)) &
                         + x(ixO^S, 1)**2 * w(ixO^S, Bvec(2))*w(ixO^S, W_vel(2)) &
                         + x(ixO^S, 1)**2 * dsin(x(ixO^S, 2))**2 * w(ixO^S, Bvec(3)) * w(ixO^S, W_vel(3))
    w(ixO^S, mag_press_) = 0.5d0 * ( w(ixO^S, B2_) + w(ixO^S, mag_press_)**2 ) !/ lfac(ixO^S)**2
