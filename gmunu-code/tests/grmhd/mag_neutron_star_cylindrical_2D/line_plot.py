...
ds_final = yt.load('output0090.dat', geometry_override='cylindrical', unit_system='code')


# cutting the x-axis through the y=0,z=0 
z_cut = 0.

plotdata_initial = ds_initial.ortho_ray( 0, (z_cut, 0) )
plotdata_final = ds_final.ortho_ray( 0, (z_cut, 0) )
...
gamma_initial = gamma_initial + np.array((plotdata_initial['r'])**2 * plotdata_initial['W_vel2']**2 + (plotdata_initial['r'])**2*plotdata_initial['W_vel3']**2)
gamma_initial = gamma_initial * np.array(plotdata_initial['psi']**4)
gamma_initial = np.sqrt(1.0 + gamma_initial)

sqrt_vphi2_initial = np.array(plotdata_initial['psi']**2*plotdata_initial['r']*plotdata_initial['W_vel3'])
sqrt_vphi2_initial = sqrt_vphi2_initial / gamma_initial

gamma_final = np.array(plotdata_final['W_vel1']**2)
gamma_final = gamma_final + np.array((plotdata_final['r'])**2 * plotdata_final['W_vel2']**2 + (plotdata_final['r'])**2*plotdata_final['W_vel3']**2)
gamma_final = gamma_final * np.array(plotdata_final['psi']**4)
gamma_final = np.sqrt(1.0 + gamma_final)

sqrt_vphi2_final = np.array(plotdata_final['psi']**2*plotdata_final['r']*plotdata_final['W_vel3'])
...
#plt.plot(np.array(plotdata_final['r'][srt]), np.array(gamma_final[srt]), 'r.', label='$ \\widetilde{ \\sqrt{v^\\phi v_\\phi} } $' )
#plt.plot(np.array(plotdata_initial['r'][srt]), np.array(gamma_initial), color='green', linestyle='-', linewidth = 1)
...
plt.plot(np.array(plotdata_final['r'][srt]), np.array(sqrt_vphi2_final[srt]), 'b.', label='$ \\widetilde{ \\sqrt{v^\\phi v_\\phi} } $' )
plt.plot(np.array(plotdata_initial['r'][srt]), np.array(sqrt_vphi2_initial[srt]), color='black', linestyle='-', linewidth = 1)
...
with rc_context({'mathtext.fontset': 'stix'}):
  plt.savefig(savepath+'slices.png')
