...
for i, data_set in enumerate(data_sets):
  # Load the dataset
  ds = yt.load(data_set, unit_system='code')
  # get time
  # get 1D data set
  slice_data = ds.ortho_ray( 'x', (y_cut, z_cut) )
  # locate where is the data
  data_loc = min(range(len(slice_data['x'])), key=lambda i: abs(float(slice_data['x'][i])-x_cut))
