...
ds_final = yt.load('output0999.dat', geometry_override='spherical', unit_system='code')


# cutting the x-axis through the y=0,z=0 
plotdata_initial = ds_initial.ortho_ray( 0, (np.pi/4.0, 0) )
plotdata_final = ds_final.ortho_ray( 0, (np.pi/4.0, 0) )
...
plt.plot(np.array(plotdata_final['r'][srt]), np.array((plotdata_final['rho'][srt]) / (plotdata_final['rho'][0]) ), 'r.', label='$\\rho / \\rho_c$' )
