module mod_usr
  ...
  double precision, parameter            :: rho_out = 1.0d-3
  double precision, parameter            :: p_in = 1.0d0
  double precision, parameter            :: p_out = 1.0d-3
...
contains
  ...
  subroutine rm_init_one_grid(ixI^L,ixO^L,w,x)
    ...
    if (stagger_grid) then
       ...
       block%conss(ixI^S,2) = 0.0d0
    end if

    w(ixO^S,Evec(1))    = 0.0d0
    w(ixO^S,Evec(2))    = 0.0d0
    w(ixO^S,Evec(3))    = 0.0d0
