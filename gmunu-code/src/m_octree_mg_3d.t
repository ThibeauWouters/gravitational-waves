...
contains
  ...
  subroutine box_gs_lalp(mg, id, nc, redblack_cntr)
    ...
    integer                   :: i, j, k, i0, di
    integer                   :: lo(3), hi(3), pm(3)

    real(dp)                  :: u(-1:1, 3)

    logical                   :: redblack
    real(dp)                  :: idr2(3), dr(3), dr2(3)
    real(dp)                  :: Lop, dLdu
  ...
  subroutine box_clalp(mg, id, nc, i_out)
    ...
    real(dp)                  :: face_coeff(0:1, 3) = 1.0d0
    real(dp)                  :: pre_fac(3) = 1.0d0
    real(dp)                  :: u(-1:1, 3)
  ...
  subroutine box_clbeta(mg, id, nc, i_dir, i_out)
    ...
    real(dp)                  :: face_coeff(0:1, 3) = 1.0_dp
    real(dp)                  :: pre_fac(3) = 1.0_dp
    real(dp)                  :: u(-1:1, 3, 3)
    real(dp)                  :: u_tmp_1(-1:1)
    real(dp)                  :: du(3, 3)
    real(dp)                  :: d2u(3, 3, 3)
  ...
  subroutine Laplacian(lop, u_in, idr2, dLdu, face_coeff_in, pre_fac_in)
    ...
    real(dp), intent(in)                :: u_in(-1:1, 3)
    real(dp), intent(in)                :: idr2(3)
    real(dp), intent(in), optional      :: face_coeff_in(0:1, 3)
    real(dp), intent(in), optional      :: pre_fac_in(3)

    integer                             :: idir
    real(dp)                            :: u(-1:1, 3) = 0.0d0
    real(dp)                            :: face_coeff(0:1, 3) = 1.0d0
    real(dp)                            :: pre_fac(3) = 1.0d0
       ...
       if ( nb <= 2 ) then
          ...
          do k = 1, nc
             ...
             do j = 1, nc
                ...
                tmp = get_tmp( mg%operator_type==mg_cfc_alp ) * mg%boxes(id)%cc(i+pm_one, j, k, iv) &
          ...
          do k = 1, nc
             ...
             do i = 1, nc
                x(1) = mg%boxes(id)%r_min(1) + dx(1) * (i-0.5_dp)
                tmp = get_tmp( mg%operator_type==mg_cfc_alp ) * mg%boxes(id)%cc(i, j+pm_one, k, iv) &
          ...
          do j = 1, nc
             ...
             do i = 1, nc
                x(1) = mg%boxes(id)%r_min(1) + dx(1) * (i-0.5_dp)
                tmp = get_tmp( mg%operator_type==mg_cfc_alp ) * mg%boxes(id)%cc(i, j, k+pm_one, iv) &
                      + x(1) * ( mg%boxes(id)%cc(i+1, j, k, iv) - mg%boxes(id)%cc(i-1, j, k, iv) ) / (2.0_dp * dx(1)) &
                      + x(2) * ( mg%boxes(id)%cc(i, j+1, k, iv) - mg%boxes(id)%cc(i, j-1, k, iv) ) / (2.0_dp * dx(2)) 
    ...
    CONTAINS
       real(dp) function get_tmp(flag) 
          ...
          if (flag) then
             ...
             ! LHS = (u+1)(2 - 1/psi) - psi
             get_tmp = ( mg%boxes(id)%cc(i, j, k, iv) + 1.0_dp ) &
                  * ( 2.0_dp - 1.0_dp / mg%boxes(id)%cc(i, j, k, mg_itmp2) ) &
                  - mg%boxes(id)%cc(i, j, k, mg_itmp2)
          else
             get_tmp = mg%boxes(id)%cc(i, j, k, iv) 
          end if
