module mod_gremhd_phys_divb_mg
  ...
  public :: gremhd_initial_clean_divb_multigrid
...
contains
  ...
  subroutine gremhd_initial_clean_divb_multigrid(iit,qt)
    use mod_global_parameters
    ...
    call gremhd_clean_divb_mg()
    if (mype==0) then
      write(*, '(A,ES9.2,A)') ' Cleaned B field'
    end if
    divB_mg_tol = divB_mg_tol_tmp
  end subroutine gremhd_initial_clean_divb_multigrid

  subroutine gremhd_clean_divb_multigrid(iit,qt)
    integer, intent(in)          :: iit
    double precision, intent(in) :: qt
    call gremhd_clean_divb_mg()
  ...
  subroutine gremhd_clean_divb_mg()
    use mod_ghostcells_update, only: getbc
    use mod_forest
    use mod_global_parameters
    use mod_multigrid_coupling
    ...
    do iigrid = 1, igridstail
       ...
       call gremhd_get_divb(ixG^LL, ixM^LL, ps(igrid), tmp(ixG^T))
  ...
  end subroutine gremhd_clean_divb_mg
