...
contains
  ...
  subroutine PPMlimitervar(ixI^L,ix^L,idims,q,qCT,qLC,qRC)
    ...
    ! fixme: the following part seems problematic
  ...
  subroutine PPMlimiter(ixI^L,ix^L,idims,w,wCT,wLC,wRC,fattening)
    ...
    integer, intent(in)             :: ixI^L, ix^L, idims
    double precision, intent(in)    :: w(ixI^S),wCT(ixI^S)

    double precision, intent(inout) :: wRC(ixG^T),wLC(ixG^T) 
    logical, intent(in)             :: fattening

    double precision,dimension(ixG^T)  :: dwC,d2wC,ldw
    double precision,dimension(ixG^T)  :: wMin,wMax,tmp
    ...
    ! Eq. 66,  Miller and Colella 2002, JCP 183, 26 
    wLC(ixO^S)=wLC(ixO^S)+half*dwC(ixO^S)&
         +(ldw(ixO^S)-ldw(ixR^S))/6.0d0

    wRC(ixL^S)=wRC(ixL^S)-(half*dwC(ixL^S)&
         -(ldw(ixL^S)-ldw(ixO^S))/6.0d0)

    ! fixme: the following part seems problematic
    ! make sure that min wCT(i)<wLC(i)<wCT(i+1) same for wRC(i)
    !call extremaw(rec_from,rec_to,ixI^L,kxC^L,wCT,1,wMax,wMin)

    ! Eq. B8, page 217, Mignone et al 2005, ApJS
    !wRC(ixL^S)=max(wMin(ixO^S)&
    !     ,min(wMax(ixO^S),wRC(ixL^S))) 
    !wLC(ixO^S)=max(wMin(ixO^S)&
    !     ,min(wMax(ixO^S),wLC(ixO^S)))


    ! Eq. B9, page 217, Mignone et al 2005, ApJS
    where((wRC(ixL^S)-wCT(ixO^S))&
         *(wCT(ixO^S)-wLC(ixO^S))<=zero)
       wRC(ixL^S)=wCT(ixO^S)
       wLC(ixO^S)=wCT(ixO^S)
    end where

    wMax(ixO^S)=(wLC(ixO^S)-wRC(ixL^S))*&
         (wCT(ixO^S)-&
         (wLC(ixO^S)+wRC(ixL^S))/2.0d0)
    wMin(ixO^S)=(wLC(ixO^S)-wRC(ixL^S))**2.0d0/6.0d0
    tmp(ixL^S)=wRC(ixL^S)
    ! Eq. B10, page 218, Mignone et al 2005, ApJS
    where(wMax(hxR^S)>wMin(hxR^S))
       wRC(hxC^S)= 3.0d0*wCT(hxR^S)&
            -2.0d0*wLC(hxR^S)
    endwhere
    ! Eq. B11, page 218, Mignone et al 2005, ApJS
    where(wMax(hxC^S)<-wMin(hxC^S))
       wLC(hxC^S)= 3.0d0*wCT(hxC^S)&
            -2.0d0*tmp(hxL^S)
    endwhere
    ...
    if(flatcd.and.fattening)then
       ...
       if(any(kappa*aa(kxC^S)>=ab(kxC^S)))then
          where(kappa*aa(kxC^S)>=ab(kxC^S).and. dabs(dwC(kxC^S))>smalldouble)
             wMax(kxC^S) = wCT(kxR^S)-2.0d0*wCT(kxC^S)+wCT(kxL^S)
          end where

          where(wMax(ixR^S)*wMax(ixL^S)<zero .and.&
               dabs(wCT(ixR^S)-wCT(ixL^S))&
               -eps*min(dabs(wCT(ixR^S)),dabs(wCT(ixL^S)))>zero &
               .and. kappa*aa(ixO^S)>=ab(ixO^S)&
               .and. dabs(dwC(ixO^S))>smalldouble)

             ac(ixO^S)=(wCT(ixLL^S)-wCT(ixRR^S)+4.0d0*dwC(ixO^S))&
                  /(12.0d0*dwC(ixO^S))
             wMin(ixO^S)=max(zero,min(eta1*(ac(ixO^S)-eta2),one))
          elsewhere
             wMin(ixO^S)=zero
          end where

          where(wMin(hxC^S)>zero)
             wLC(hxC^S) = wLC(hxC^S)*(one-wMin(hxC^S))&
                  +(wCT(hxC^S)+half*ldw(hxC^S))*wMin(hxC^S)
          end where
          where(wMin(hxR^S)>zero)
             wRC(hxC^S) = wRC(hxC^S)*(one-wMin(hxR^S))&
                  +(wCT(hxR^S)-half*ldw(hxR^S))*wMin(hxR^S)
          end where
       endif
    endif
    ...
    if(flatsh.and.fattening)then
       ...
       where(dabs(ab(ixO^S)-one)>smalldouble)
          wMax(ixO^S) = (one-ab(ixO^S))*wCT(ixO^S)
       endwhere

       where(dabs(ab(hxC^S)-one)>smalldouble)
          wLC(hxC^S) = ab(hxC^S)*wLC(hxC^S)+wMax(hxC^S)
       endwhere

       where(dabs(ab(hxR^S)-one)>smalldouble)
          wRC(hxC^S) = ab(hxR^S)*wRC(hxC^S)+wMax(hxR^S)
       endwhere
    endif
  ...
  subroutine extremaw(ixI^L,ixO^L,w,nshift,wMax,wMin)
    ...
    do ishift=1,nshift
      ...
      if (ishift==1) then
        wMax(ixO^S)= &
             max(w(ixO^S),w(ixsR^S),w(ixsL^S))
        wMin(ixO^S)= &
             min(w(ixO^S),w(ixsR^S),w(ixsL^S))
      else
        wMax(ixO^S)= &
             max(wMax(ixO^S),w(ixsR^S),w(ixsL^S))
        wMin(ixO^S)= &
             min(wMin(ixO^S),w(ixsR^S),w(ixsL^S))
      end if
      {^NOONED
      idims=1
      jdims=idims+1
