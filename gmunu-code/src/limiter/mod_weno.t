...
!    u3_coeff(1:3) = (/ 3.d0/8.d0, 6.d0/8.d0, -1.d0/8.d0 /)
  ...
  subroutine WENO5NMlimiter(ixI^L,iL^L,idims,dxdim,w,wLC,wRC,var)
    ...
    wc(iM^S) = (block%dx(iMp^S,idims) * w(iM^S) + block%dx(iM^S,idims) *  w(iMp^S)) / &
                 (block%dx(iMp^S,idims) + block%dx(iM^S,idims))
    wd(iL^S) = ((2.d0 * block%dx(iLm^S,idims) + block%dx(iLmm^S,idims)) * w(iLm^S) - block%dx(iLm^S,idims) * w(iLmm^S)) / &
                 (block%dx(iLmm^S,idims) + block%dx(iLm^S,idims))
    we(iL^S) = ((2.d0 * block%dx(iLpp^S,idims) + block%dx(iLppp^S,idims)) * w(iLpp^S) - block%dx(iLpp^S,idims) * w(iLppp^S)) / &
