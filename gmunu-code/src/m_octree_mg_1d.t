...
contains
  ...
  subroutine box_gs_lalp(mg, id, nc, redblack_cntr)
    ...
    integer                   :: i, i0, di
    integer                   :: lo(1), hi(1), pm(1)

    real(dp)                  :: u(-1:1, 1)

    logical                   :: redblack
    real(dp)                  :: idr2(1), dr(1), dr2(1)
    real(dp)                  :: Lop, dLdu
  ...
  subroutine box_clalp(mg, id, nc, i_out)
    ...
    real(dp)                  :: face_coeff(0:1, 1) = 1.0d0
    real(dp)                  :: pre_fac(1) = 1.0d0
    real(dp)                  :: u(-1:1, 1)
  ...
  subroutine box_clbeta(mg, id, nc, i_dir, i_out)
    ...
    real(dp)                  :: face_coeff(0:1, 1) = 1.0_dp
    real(dp)                  :: pre_fac(1) = 1.0_dp
    real(dp)                  :: u(-1:1, 1, 3)
    real(dp)                  :: u_tmp_1(-1:1)
    real(dp)                  :: du(3, 1)
    real(dp)                  :: d2u(3, 1, 1)
  ...
  subroutine Laplacian(lop, u_in, idr2, dLdu, face_coeff_in, pre_fac_in)
    ...
    real(dp), intent(in)                :: u_in(-1:1, 1)
    real(dp), intent(in)                :: idr2(1)
    real(dp), intent(in), optional      :: face_coeff_in(0:1, 1)
    real(dp), intent(in), optional      :: pre_fac_in(1)

    integer                             :: idir
    real(dp)                            :: u(-1:1, 1) = 0.0d0
    real(dp)                            :: face_coeff(0:1, 1) = 1.0d0
    real(dp)                            :: pre_fac(1) = 1.0d0
    ...
    CONTAINS
       real(dp) function get_tmp(flag) 
          ...
          if (flag) then
             ...
             ! LHS = (u+1)(2 - 1/psi) - psi
             get_tmp = ( mg%boxes(id)%cc(i, iv) + 1.0_dp ) &
                  * ( 2.0_dp - 1.0_dp / mg%boxes(id)%cc(i, mg_itmp2) ) &
                  - mg%boxes(id)%cc(i, mg_itmp2)
          else
             get_tmp = mg%boxes(id)%cc(i, iv) 
          end if
  ...
  real(dp) function max_residual_lvl(mg, lvl)
    ...
    do i = 1, size(mg%lvls(lvl)%my_ids)
       ...
       if (.not.mg%vector_equation) then
          ...
          do idir = 1,3
             res = max(res, &
               maxval(abs(mg%boxes(id)%cc(1:nc, mg_vec_ires(idir)))) )
          end do
       end if
  ...
  subroutine update_coarse(mg, lvl)
    ...
    if (.not.mg%vector_equation) then
       ...
       do i = 1, size(mg%lvls(lvl-1)%my_parents)
          ...
          do idir = 1,3
             ! Add the fine grid residual to rhs
             mg%boxes(id)%cc(1:nc_c, mg_vec_irhs(idir)) = &
                  mg%boxes(id)%cc(1:nc_c, mg_vec_irhs(idir)) + &
                  mg%boxes(id)%cc(1:nc_c, mg_vec_ires(idir))
          end do
  ...
  subroutine residual_box(mg, id, nc)
    ...
    if (.not.mg%vector_equation) then
       ...
       do idir = 1, 3
          ...
          mg%boxes(id)%cc(1:nc, mg_vec_ires(idir)) = &
               mg%boxes(id)%cc(1:nc, mg_vec_irhs(idir)) &
               - mg%boxes(id)%cc(1:nc, mg_vec_ires(idir))
       end do
    end if
