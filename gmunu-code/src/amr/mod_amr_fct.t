...
contains
  ...
  subroutine prolong_2nd_stg(sCo,sFi,ixCo^Lin,ixFi^Lin,dxCo^D,xComin^D,dxFi^D,xFimin^D,ghost,fine_^Lin)
    ...
    double precision :: bfluxCo(sCo%ixGs^S,nconss),bfluxFi(sFi%ixGs^S,nconss)
    ...
    associate(wCos=>sCo%conss, wFis=>sFi%conss,wCo=>sCo%prim, wFi=>sFi%prim)
          ...
          if (refine(ineighbor,ipe_neighbor)) then
            ...
            if (iside==1) then !! left side
              pface(iside,^D,igrid)%face(1^D%1:nx^DD)=&
                ps(igrid)%conss(ixMlo^D-1^D%ixM^T,^D)
            else !! right side
              pface(iside,^D,igrid)%face(1^D%1:nx^DD)=&
                ps(igrid)%conss(ixMhi^D^D%ixM^T,^D)
            end if
  ...
  subroutine already_fine(sFi,ichild,fine_^L)
   ...
   {do iside=1,2
      ...
      if (ineighbor>0) then
        if (iside==1) then ! left side
          if (ipe_neighbor==mype) then
            sFi%conss(ixMlo^D-1^D%ixM^T,^D)=pface(iotherside,^D,ineighbor)%face(1^D%1:nx^DD)
          else
            ibufnext=ibuf_recv+isize(^D)
            sFi%conss(ixMlo^D-1^D%ixM^T,^D)=reshape(&
                 source=recvbuffer(ibuf_recv:ibufnext-1),&
                 shape=shape(sFi%conss(ixMlo^D-1^D%ixM^T,^D)))
            ibuf_recv=ibufnext
          end if
          fine_min^D=.true.
        else ! right side
          if (ipe_neighbor==mype) then
            sFi%conss(ixMhi^D^D%ixM^T,^D)=pface(iotherside,^D,ineighbor)%face(1^D%1:nx^DD)
          else
            ibufnext=ibuf_recv+isize(^D)
            sFi%conss(ixMhi^D^D%ixM^T,^D)=reshape(&
                 source=recvbuffer(ibuf_recv:ibufnext-1),&
                 shape=shape(sFi%conss(ixMhi^D^D%ixM^T,^D)))
            ibuf_recv=ibufnext
          end if
          fine_max^D=.true.
        end if
