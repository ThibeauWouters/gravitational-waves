    ...
    contains
      ...
      subroutine lb_recv
        ...
        if (phyboundblock(recv_igrid)) then
           call MPI_IRECV(ps(recv_igrid)%prim,1,type_block,send_ipe,itag, &
                          icomm,recvrequest(irecv),ierrmpi)
        else
           call MPI_IRECV(ps(recv_igrid)%prim,1,type_block_io,send_ipe,itag, &
                          icomm,recvrequest(irecv),ierrmpi)
        end if
        ...
        if(stagger_grid) then
          ...
          call MPI_IRECV(ps(recv_igrid)%conss,1,type_block_io_stg,send_ipe,itag, &
      ...
      subroutine lb_send
        ...
        if (phyboundblock(send_igrid)) then
           call MPI_ISEND(ps(send_igrid)%prim,1,type_block,recv_ipe,itag, &
                          icomm,sendrequest(isend),ierrmpi)
        else
           call MPI_ISEND(ps(send_igrid)%prim,1,type_block_io,recv_ipe,itag, &
                          icomm,sendrequest(isend),ierrmpi)
        end if
        }{#IFNDEF EVOLVINGBOUNDARY
        call MPI_ISEND(ps(send_igrid)%prim,1,type_block_io,recv_ipe,itag, &
        ...
        if(stagger_grid) then
          itag=recv_igrid+max_blocks
          call MPI_ISEND(ps(send_igrid)%conss,1,type_block_io_stg,recv_ipe,itag, &
