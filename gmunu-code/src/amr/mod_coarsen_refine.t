  ...
  subroutine amr_coarsen_refine
    ...
    if ( fix_small_values ) then
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          call phys_handle_small_values(ps(igrid)%prim(ixG^T,1:nprim),ps(igrid)%x(ixG^T,1:ndim),ixG^LL,ixM^LL, .True., 'amr_coarsen_refine')
       end do
       !$OMP END PARALLEL DO
    end if
    ...
    if (time_advance) then
       call getbc(global_time+dt,0.d0,ps,1,nprim)
    else
    ...
    {do ic^DB=1,2\}
       ...
       if (ipeFi==mype) then
          ...
          if (ipe==mype) then
             ...
             isend=isend+1
             call MPI_ISEND(psc(igridFi)%prim,1,type_coarse_block,ipe,itag, &
                            icomm,sendrequest(isend),ierrmpi)
             if(stagger_grid) then
               do idir=1,ndim
                 itag_stg=(npe+ipeFi+1)*max_blocks+igridFi*(ndir-1+idir)
                 call MPI_ISEND(psc(igridFi)%conss,1,type_coarse_block_stg(idir,ic^D),ipe,itag_stg, &
          ...
          if (ipe==mype) then
             itag=ipeFi*max_blocks+igridFi
             irecv=irecv+1
             call MPI_IRECV(ps(igrid)%prim,1,type_sub_block(ic^D),ipeFi,itag, &
                            icomm,recvrequest(irecv),ierrmpi)
             if(stagger_grid) then
               do idir=1,ndim
                 itag_stg=(npe+ipeFi+1)*max_blocks+igridFi*(ndir-1+idir)
                 call MPI_IRECV(ps(igrid)%conss,1,type_sub_block_stg(idir,ic^D),ipeFi,itag_stg, &
