...
subroutine errest
  ...
  !$OMP PARALLEL DO PRIVATE(igrid)
     do iigrid=1,igridstail; igrid=igrids(iigrid);
        block=>ps(igrid)
        call compare1_grid(igrid,pso(igrid)%prim,ps(igrid)%prim)
     end do
  !$OMP END PARALLEL DO
  case (2)
  ...
  do iigrid=1,igridstail; igrid=igrids(iigrid);
     block=>ps(igrid)
     call forcedrefine_grid(igrid,ps(igrid)%prim)
  end do
  !$OMP END PARALLEL DO
  ...
  do iflag=1,nprim+1
     ...
     if (iflag > nprim) then
        if (.not. associated(usr_var_for_errest)) then
           ...
           call usr_var_for_errest(ixG^LL,ixG^LL,iflag,ps(igrid)%prim,ps(igrid)%x,tmp1)
        end if
     end if
...
subroutine lohner_orig_grid(igrid)
  ...
  do iflag=1,nprim+1
     ...
     do idims=1,ndim
        ...
        if (iflag<=nprim) then
          if (logflag(iflag)) then
            dp(ix^S)=dlog10(ps(igrid)%prim(jx^S,iflag))-dlog10(ps(igrid)%prim(ix^S,iflag))
            dm(ix^S)=dlog10(ps(igrid)%prim(ix^S,iflag))-dlog10(ps(igrid)%prim(hx^S,iflag))
            dref(ixM^T)=dabs(dlog10(ps(igrid)%prim(jx^S,iflag)))&
                       + 2.0d0 * dabs(dlog10(ps(igrid)%prim(ixM^T,iflag))) &
                       + dabs(dlog10(ps(igrid)%prim(hx^S,iflag)))
          else
            dp(ix^S)=ps(igrid)%prim(jx^S,iflag)-ps(igrid)%prim(ix^S,iflag)
            dm(ix^S)=ps(igrid)%prim(ix^S,iflag)-ps(igrid)%prim(hx^S,iflag)
            dref(ixM^T)=dabs(ps(igrid)%prim(jx^S,iflag))+2.0d0*dabs(ps(igrid)%prim(ixM^T,iflag)) &
                        +dabs(ps(igrid)%prim(hx^S,iflag))
          end if
        else
          if (logflag(iflag)) then
  ...
  {do ix^DB=ixMlo^DB,ixMhi^DB\}
     ...
     if (associated(usr_refine_threshold)) then
        wtol(1:nprim)   = ps(igrid)%prim(ix^D,1:nprim)
        xtol(1:ndim) = ps(igrid)%x(ix^D,1:ndim)
        call usr_refine_threshold(wtol, xtol, threshold, global_time, level)
     end if
...
subroutine compare1_grid(igrid,wold,w)
  ...
  integer, intent(in) :: igrid
  double precision, intent(in) :: wold(ixG^T,1:nprim), w(ixG^T,1:nprim)

  integer :: ix^D, iflag, level
  double precision :: epsilon, threshold, wtol(1:nprim), xtol(1:ndim)
  double precision :: average, error
  double precision :: averages(nprim)
  logical, dimension(ixG^T) :: refineflag, coarsenflag
...
subroutine forcedrefine_grid(igrid,w)
  ...
  integer, intent(in) :: igrid
  double precision, intent(in) :: w(ixG^T,nprim)
