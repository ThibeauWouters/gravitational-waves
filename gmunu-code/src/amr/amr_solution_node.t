...
subroutine alloc_node(igrid)
  ...
  ps(igrid)%prim=smalldouble
  ps1(igrid)%prim=smalldouble
  psc(igrid)%prim=smalldouble
  ps(igrid)%cons=smalldouble
  ps1(igrid)%cons=smalldouble
  psc(igrid)%cons=smalldouble
 ...
 {do ix=ixGextmin^D,ixGextmax^D
  ...
  select case (coordinate)
    case (Cartesian)
      ...
      ps(igrid)%dlE(ixGext^S,1:ndim)=ps(igrid)%dx(ixGext^S,1:ndim)
    ...
    case (spherical)
      ...
      {^IFTHREED ps(igrid)%dsC(ixGext^S,3)=(xext(ixGext^S,1)+0.5d0*ps(igrid)%dx(ixGext^S,1))*&
                                       dsin(xext(ixGext^S,2)+0.5d0*ps(igrid)%dx(ixGext^S,2))*&
                                       ps(igrid)%dx(ixGext^S,3)}
      ps(igrid)%dlE(ixGext^S,1)=(xext(ixGext^S,1)**2+ps(igrid)%dx(ixGext^S,1)**2/12.0d0)*&
              ps(igrid)%dx(ixGext^S,1) {^NOONED * dabs(dsin(xext(ixGext^S,2)+0.5d0*ps(igrid)%dx(ixGext^S,2))) }
      {^NOONED   ps(igrid)%dlE(ixGext^S,2)=(xext(ixGext^S,1)+0.5d0*ps(igrid)%dx(ixGext^S,1))**2 * &
                      2.0d0*dabs(dsin(xext(ixGext^S,2)))*dsin(0.5d0*ps(igrid)%dx(ixGext^S,2)) }
      {^IFTHREED ps(igrid)%dlE(ixGext^S,3)=(xext(ixGext^S,1)+0.5d0*ps(igrid)%dx(ixGext^S,1))**2 *&
                                       dsin(xext(ixGext^S,2)+0.5d0*ps(igrid)%dx(ixGext^S,2))*&
                                       ps(igrid)%dx(ixGext^S,3)}
    case (cylindrical)
      ...
      ps(igrid)%dlE(ixGext^S,r_)=(xext(ixGext^S,1)+&
                   0.5d0*ps(igrid)%dx(ixGext^S,1))*ps(igrid)%dx(ixGext^S,r_)
      if(z_>0.and.z_<=ndim) then
        ps(igrid)%ds(ixGext^S,z_)=ps(igrid)%dx(ixGext^S,z_)
        ps(igrid)%dsC(ixGext^S,z_)=ps(igrid)%dx(ixGext^S,z_)
        ps(igrid)%dlE(ixGext^S,z_)=(xext(ixGext^S,1)+&
                   0.5d0*ps(igrid)%dx(ixGext^S,1))*ps(igrid)%dx(ixGext^S,z_)
        if(phi_>z_.and.ndir>ndim) then
          ps(igrid)%dsC(ixGext^S,phi_)= 1.0d0 !xext(ixGext^S,1)+0.5d0*ps(igrid)%dx(ixGext^S,1)
        end if
      end if
      if(phi_>0.and.phi_<=ndim) then
        ...
        ps(igrid)%dlE(ixGext^S,phi_)=(xext(ixGext^S,1)+&
                   0.5d0*ps(igrid)%dx(ixGext^S,1))*ps(igrid)%dx(ixGext^S,phi_)
        if(z_>phi_.and.ndir>ndim) then
           ps(igrid)%dsC(ixGext^S,z_)=1.d0
           ps(igrid)%dlE(ixGext^S,z_)=xext(ixGext^S,1)+0.5d0*ps(igrid)%dx(ixGext^S,1)
        end if
      end if
    case default
...
subroutine alloc_state(igrid, s, ixG^L, ixGext^L, alloc_once_for_ps)
  ...
  if(stagger_grid) then
    allocate(s%conss(ixGs^S,1:nconss))
    s%conss=0.d0
    s%ixGs^L=ixGs^L;
  end if
...
subroutine dealloc_state(igrid, s,dealloc_x)
  ...
  if(dealloc_x) then
    ...
    nullify(s%x,s%xbar,s%dx,s%ds,s%dsC,s%dlE,s%dvolume,s%surfaceC,s%surface,s%christoffel)
