!> GMUNU solves a set of hyperbolic equations
...
program gmunu
  ...
  if (restart_from_file /= undefined) then
     ...
     ! get input data from previous GMUNU run
  ...
  if (mype==0) then
     print*,'-------------------------------------------------------------------------------'
     write(*,'(a,f17.3,a)')' Finished GMUNU in : ',MPI_WTIME()-time0,' sec'
     print*,'-------------------------------------------------------------------------------'
  end if
...
contains
  ...
  subroutine timeintegration()
    ...
    time_evol : do
       ...
       if (any(save_file)) then
         if(associated(usr_modify_output)) then
           ...
           do iigrid=1,igridstail; igrid=igrids(iigrid);
             ...
             block=>ps(igrid)
             call usr_modify_output(ixG^LL,ixM^LL,global_time,ps(igrid)%prim,ps(igrid)%x)
           end do
         end if
         time_write=0.d0
         do ifile=nfile,1,-1
       ...
       ! folder as the executable gmunu
       ...
       if (crashall) then
         do iigrid=1,igridstail; igrid=igrids(iigrid);
           ps(igrid)%prim=pso(igrid)%prim
         end do
         call saveamrfile(1)
         call saveamrfile(2)
         if(mype==0) write(*,*) "Error: small value/ NaN encountered, run crash."
