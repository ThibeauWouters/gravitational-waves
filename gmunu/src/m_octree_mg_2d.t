...
!> Module which contains multigrid procedures for the L operator for (psi - 1)
  ...
  integer, parameter, public :: mg_cfc_psi = 5
  !> Indicates the (alp-1) equation for CFC
  integer, parameter, public :: mg_cfc_alp = 6
  !> Indicates the orthnormal (beta) equation for CFC
  integer, parameter, public :: mg_cfc_beta = 7
  ...
  integer, parameter, public :: mg_iveps = 7
  ...
  interface
     ...
     !> Subroutine that performs A * cc(..., i_in) = cc(..., i_out)
     subroutine mg_vec_box_op(mg, id, nc, i_dir, i_out)
       import
       type(mg_t), intent(inout) :: mg
       integer, intent(in)       :: id
       integer, intent(in)       :: nc
...
!> Module for implicitly solving diffusion equations
  ...
  !! File ../src/m_finite_difference.f90
...
contains
  ...
  subroutine box_gs_lalp(mg, id, nc, redblack_cntr)
    ...
    integer                   :: i, j, i0, di
    integer                   :: lo(2), hi(2), pm(2)

    real(dp)                  :: u(-1:1, 2)

    logical                   :: redblack
    real(dp)                  :: idr2(2), dr(2), dr2(2)
    real(dp)                  :: Lop, dLdu
  ...
  subroutine box_clalp(mg, id, nc, i_out)
    ...
    real(dp)                  :: face_coeff(0:1, 2) = 1.0d0
    real(dp)                  :: pre_fac(2) = 1.0d0
    real(dp)                  :: u(-1:1, 2)
  ...
  subroutine box_clbeta(mg, id, nc, i_dir, i_out)
    ...
    real(dp)                  :: face_coeff(0:1, 2) = 1.0_dp
    real(dp)                  :: pre_fac(2) = 1.0_dp
    real(dp)                  :: u(-1:1, 2, 3)
    real(dp)                  :: u_tmp_1(-1:1)
    real(dp)                  :: du(3, 2)
    real(dp)                  :: d2u(3, 2, 2)
  ...
  subroutine box_gs_clpsi(mg, id, nc, redblack_cntr)
    ...
    where ( mg%boxes(id)%r_min + 0.5_dp * nc * mg%boxes(id)%dr.ge.0 .and. mg%symm )
      pm = 1
      lo = 1
      hi = nc
    else where
  ...
  subroutine Laplacian(lop, u_in, idr2, dLdu, face_coeff_in, pre_fac_in)
    ...
    real(dp), intent(in)                :: u_in(-1:1, 2)
    real(dp), intent(in)                :: idr2(2)
    real(dp), intent(in), optional      :: face_coeff_in(0:1, 2)
    real(dp), intent(in), optional      :: pre_fac_in(2)

    integer                             :: idir
    real(dp)                            :: u(-1:1, 2) = 0.0d0
    real(dp)                            :: face_coeff(0:1, 2) = 1.0d0
    real(dp)                            :: pre_fac(2) = 1.0d0
    ...
    case (mg_cylindrical)
       if ( nb <= 2 ) then
          ...
          do j = 1, nc
             ...
             tmp = get_tmp( mg%operator_type==mg_cfc_alp ) * mg%boxes(id)%cc(i+pm_one, j, iv) &
          ...
          do i = 1, nc
             x(1) = mg%boxes(id)%r_min(1) + dx(1) * (i-0.5_dp)
             tmp = get_tmp( mg%operator_type==mg_cfc_alp ) * mg%boxes(id)%cc(i, j+pm_one, iv) &
    ...
    CONTAINS
       real(dp) function get_tmp(flag) 
          ...
          if (flag) then
             ...
             ! LHS = (u+1)(2 - 1/psi) - psi
             get_tmp = ( mg%boxes(id)%cc(i, j, iv) + 1.0_dp ) &
                  * ( 2.0_dp - 1.0_dp / mg%boxes(id)%cc(i, j, mg_itmp2) ) &
                  - mg%boxes(id)%cc(i, j, mg_itmp2)
          else
             get_tmp = mg%boxes(id)%cc(i, j, iv) 
          end if
...
!   end subroutine box_jacobi_lpl
    ...
    do lvl = mg%lowest_lvl, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_ids)
          ...
          if (include_ghostcells) then
             ...
             mg%boxes(id)%cc(1:nc, 1:nc, iv) = &
                  mg%boxes(id)%cc(1:nc, 1:nc, iv) - mean_iv
          end if
       end do
    end do
  ...
  real(dp) function get_sum(mg, iv)
    ...
    do lvl = 1, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_leaves)
          id = mg%lvls(lvl)%my_leaves(i)
          get_sum = get_sum + w * &
               sum(mg%boxes(id)%cc(1:nc, 1:nc, iv))
       end do
    end do
  ...
  real(dp) function max_residual_lvl(mg, lvl)
    ...
    do i = 1, size(mg%lvls(lvl)%my_ids)
       ...
       if (.not.mg%vector_equation) then
          ...
          do idir = 1,3
             res = max(res, &
               maxval(abs(mg%boxes(id)%cc(1:nc, 1:nc, mg_vec_ires(idir)))) )
          end do
       end if
  ...
  subroutine update_coarse(mg, lvl)
    ...
    if (.not.mg%vector_equation) then
       ...
       do i = 1, size(mg%lvls(lvl-1)%my_parents)
          ...
          mg%boxes(id)%cc(1:nc_c, 1:nc_c, mg_irhs) = &
               mg%boxes(id)%cc(1:nc_c, 1:nc_c, mg_irhs) + &
               mg%boxes(id)%cc(1:nc_c, 1:nc_c, mg_ires)
       ...
       do i = 1, size(mg%lvls(lvl-1)%my_parents)
          ...
          do idir = 1,3
             ! Add the fine grid residual to rhs
             mg%boxes(id)%cc(1:nc_c, 1:nc_c, mg_vec_irhs(idir)) = &
                  mg%boxes(id)%cc(1:nc_c, 1:nc_c, mg_vec_irhs(idir)) + &
                  mg%boxes(id)%cc(1:nc_c, 1:nc_c, mg_vec_ires(idir))
          end do
  ...
  subroutine residual_box(mg, id, nc)
    ...
    if (.not.mg%vector_equation) then
       ...
       mg%boxes(id)%cc(1:nc, 1:nc, mg_ires) = &
            mg%boxes(id)%cc(1:nc, 1:nc, mg_irhs) &
            - mg%boxes(id)%cc(1:nc, 1:nc, mg_ires)
    else
       do idir = 1, 3
          ...
          mg%boxes(id)%cc(1:nc, 1:nc, mg_vec_ires(idir)) = &
  ...
  subroutine prolong_onto(mg, id, nc, iv, iv_to, add, method)
    ...
    if (add) then
       mg%boxes(id)%cc(1:nc, 1:nc, iv_to) = &
