...
subroutine init_comm_types
  ...
  size_block={nxG^D*}*nprim*size_double
  ...
  if(stagger_grid) then
    ...
    size_block_io_stg={(nx^D+1)*}*nconss*size_double
  ...
  size_block_io={nx^D*}*nprim*size_double
  ...
  ^D&sizes(^D)=ixMhi^D-ixMlo^D+1;
  sizes(ndim+1)=nprim+nwauxio
  ^D&subsizes(^D)=sizes(^D);
  subsizes(ndim+1)=nprim+nwauxio
  ^D&start(^D)=0;
  start(ndim+1)=0
  call MPI_TYPE_CREATE_SUBARRAY(ndim+1,sizes,subsizes,start, &
                                MPI_ORDER_FORTRAN,MPI_DOUBLE_PRECISION, &
