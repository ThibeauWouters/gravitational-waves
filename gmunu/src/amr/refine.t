...
subroutine prolong_2nd(sCo,ixCo^L,sFi,dxCo^D,xComin^D,dxFi^D,xFimin^D,igridCo,igridFi)
  ...
  {do ixCo^DB = ixCg^LIM^DB
     ...
     {do ix^DB=ixFi^DB,ixFi^DB+1 \}
        ...
        wFi(ix^D,1:nprim) = wCo(ixCo^D,1:nprim) &
                              + {(slope(1:nprim,^D)*eta^D)+}
     {end do\}
  {end do\}
  ...
  if(fix_small_values) call phys_handle_small_values(wFi(ixG^T,1:nprim),sFi%x(ixG^T,1:ndim),ixG^LL,ixM^LL, .True., 'prolong_2nd')
  end associate
...
subroutine prolong_1st(wCo,ixCo^L,wFi,xFi)
  ...
  double precision, intent(in) :: wCo(ixG^T,nprim), xFi(ixG^T,1:ndim)
  double precision, intent(out) :: wFi(ixG^T,nprim)
  ...
  {do ixCo^DB = ixCo^LIM^DB
     ixFi^DB=2*(ixCo^DB-ixComin^DB)+ixMlo^DB\}
     forall(iw=1:nprim) wFi(ixFi^D:ixFi^D+1,iw)=wCo(ixCo^D,iw)
  {end do\}
