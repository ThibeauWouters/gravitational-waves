...
contains
  ...
  subroutine add_split_source(prior)
    ...
    !double precision :: w1(ixG^T,ncons)
    double precision :: qt
    ...
    case (sourcesplit_sfs)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         ...
         call addsource2(0.5d0*dt,ixG^LL,ixM^LL,1,ncons,qt,ps(igrid)%prim,qt,ps(igrid)%cons,&
    ...
    case (sourcesplit_sf)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         block=>ps(igrid)
         ^D&dxlevel(^D)=rnode(rpdx^D_,igrid);
         call addsource2(dt  ,ixG^LL,ixM^LL,1,ncons,qt,ps(igrid)%prim,qt,ps(igrid)%cons,&
              ps(igrid)%x,.true.,src_active)
      end do
      !$OMP END PARALLEL DO
    case (sourcesplit_ssf)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         block=>ps(igrid)
         ^D&dxlevel(^D)=rnode(rpdx^D_,igrid);
         call addsource2(0.5d0*dt,ixG^LL,ixG^LL,1,ncons,qt,ps(igrid)%prim,qt,ps(igrid)%cons,&
              ps(igrid)%x,.true.,src_active)
         call addsource2(dt  ,ixG^LL,ixM^LL,1,ncons,qt,ps(igrid)%prim,qt,ps(igrid)%cons,&
              ps(igrid)%x,.true.,src_active)
      end do
    ...
    case (sourcesplit_ssfss)
      ...
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
         block=>ps(igrid)
         ^D&dxlevel(^D)=rnode(rpdx^D_,igrid);
         call addsource2(0.25d0*dt,ixG^LL,ixG^LL,1,ncons,qt,ps(igrid)%prim,qt,ps(igrid)%cons,&
              ps(igrid)%x,.true.,src_active)
         call addsource2(0.5d0*dt,ixG^LL,ixM^LL,1,ncons,qt,ps(igrid)%prim,qt,ps(igrid)%cons,&
              ps(igrid)%x,.true.,src_active)
      end do
    ...
    if (src_active) then
       ...
       call getbc(qt,0.d0,ps,1,nprim,phys_req_diagonal)
    end if

  end subroutine add_split_source

  !> Add source within ixO for iws: cons=cons+qdt*S[primCT]
  subroutine addsource2(qdt,ixI^L,ixO^L,iw^LIM,qtC,primCT,qt,&
       cons,x,qsourcesplit,src_active)
    use mod_global_parameters
    use mod_physics, only: phys_add_source
    use mod_usr_methods, only: usr_source
    ! differences with VAC is in iw^LIM and in declaration of ranges for primCT,cons
    ...
    call phys_add_source(qdt,ixI^L,ixO^L,primCT,cons,x,qsourcesplit,tmp_active)

    ! user defined sources, typically explicitly added
    if ((qsourcesplit .eqv. source_split_usr) .and. associated(usr_source)) then
       tmp_active = .true.
       call usr_source(qdt,ixI^L,ixO^L,iw^LIM,qtC,primCT,qt,cons,x)
    end if
