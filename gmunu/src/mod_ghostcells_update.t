...
contains
  ...
  subroutine init_bc()
    ...
    if (stagger_grid) then
      allocate(pole_buf%conss(ixGs^T,nconss))
  ...
  subroutine getbc(time,qdt,psb,nwstart,nwbc,req_diag)
    ...
    contains
      ...
      subroutine bc_send_srl
        ...
        if(ipe_neighbor/=mype) then
          ...
          if(ipole==0) then
            ...
            if(stagger_grid) then
              ...
              do idir=1,ndim
                ...
                sendbuffer_srl(ibuf_start:ibuf_next-1)=&
                  reshape(psb(igrid)%conss(ixS^S,idir),shapes)
            ...
            call pole_buffer(pwbuf(ipwbuf)%prim,ixS^L,ixS^L,psb(igrid)%prim,ixG^LL,ixS^L)
            isend_c=isend_c+1
          ...
          if(ipole==0) then
            ...
            if(stagger_grid) then
              do idir=1,ndim
                ...
                psb(ineighbor)%conss(ixR^S,idir)=psb(igrid)%conss(ixS^S,idir)
              end do
            end if
          else
            ixS^L=ixS_srl_^L(iib^D,i^D);
            ...
            ixR^L=ixR_srl_^L(iib^D,n_i^D);
            call pole_copy(psb(ineighbor)%prim,ixG^LL,ixR^L,psb(igrid)%prim,ixG^LL,ixS^L,ipole)
            if(stagger_grid) then
              do idir=1,ndim
                ixS^L=ixS_srl_stg_^L(idir,i^D);
                ixR^L=ixR_srl_stg_^L(idir,n_i^D);
                call pole_copy_stg(psb(ineighbor)%conss,ixR^L,psb(igrid)%conss,ixS^L,idir,ipole)
              end do
            end if
          end if
        end if
            ...
            if(stagger_grid) then 
              ...
              do idir=1,ndim
                ...
                sendbuffer_r(ibuf_start:ibuf_next-1)=&
                  reshape(psc(igrid)%conss(ixS^S,idir),shapes)
                ibuf_start=ibuf_next
              end do
              isend_r=isend_r+1
          ...
          if(ipole==0) then
            ...
            if(stagger_grid) then
              do idir=1,ndim
                 ...
                 psb(ineighbor)%conss(ixR^S,idir)=psc(igrid)%conss(ixS^S,idir)
              end do
            end if
          else
            ixS^L=ixS_r_^L(iib^D,i^D);
            ...
            ixR^L=ixR_r_^L(iib^D,n_inc^D);
            call pole_copy(psb(ineighbor)%prim,ixG^LL,ixR^L,psc(igrid)%prim,ixCoG^L,ixS^L,ipole)
            if(stagger_grid) then
              do idir=1,ndim
                ixS^L=ixS_r_stg_^L(idir,i^D);
                ixR^L=ixR_r_stg_^L(idir,n_inc^D);
                !! Fill ghost cells
                call pole_copy_stg(psb(ineighbor)%conss,ixR^L,psc(igrid)%conss,ixS^L,idir,ipole)
              end do
            end if
          end if
        end if
        ...
        !! Now the special treatment of the pole is done here, at the receive step
          if (ipole==0) then    
            ...
            do idir=1,ndim
              ...
              tmp(ixS^S) = reshape(source=recvbuffer_srl(ibuf_recv_srl:ibuf_next-1),shape=shape(psb(igrid)%conss(ixS^S,idir)))       
              psb(igrid)%conss(ixR^S,idir) = tmp(ixS^S)
            ...
            do idir=1,ndim
             ...
             ixS^L=ixS_srl_stg_^L(idir,n_i^D);
             ibuf_next=ibuf_recv_srl+sizes_srl_recv_stg(idir,i^D)
             pole_buf%conss(ixS^S,idir)=reshape(source=recvbuffer_srl(ibuf_recv_srl:ibuf_next-1),&
               shape=shape(psb(igrid)%conss(ixS^S,idir)))
             ibuf_recv_srl=ibuf_next
             call pole_copy_stg(psb(igrid)%conss,ixR^L,pole_buf%conss,ixS^L,idir,ipole)
            end do
          end if
        end if
          ...
          {do ic^DB=1+int((1-i^DB)/2),2-int((1+i^DB)/2)
             ...
             if(ipe_neighbor/=mype) then
               ...
               do idir=1,ndim
                 ...
                 psb(igrid)%conss(ixR^S,idir)=reshape(source=recvbuffer_r(ibuf_recv_r:ibuf_next-1),&
                       shape=shape(psb(igrid)%conss(ixR^S,idir)))
             ...
             if(ipe_neighbor/=mype) then
              ...
              {case (^D)
               ...
               do idir=1,ndim
                 ...
                 ixR^L=ixR_r_stg_^L(idir,inc^D);
                 ibuf_next=ibuf_recv_r+sizes_r_recv_stg(idir,inc^D)
                 pole_buf%conss(ixR^S,idir)=reshape(source=recvbuffer_r(ibuf_recv_r:ibuf_next-1),&
                   shape=shape(psb(igrid)%conss(ixR^S,idir)))
                 call pole_copy_stg(psb(igrid)%conss,ixR^L,pole_buf%conss,ixR^L,idir,ipole)
                 ibuf_recv_r=ibuf_next
               end do
             end if
          {end do\}
        ...
        if(ipole==0) then
             ...
             if(ipe_neighbor==mype) then
               ...
               if(stagger_grid) then
                 do idir=1,ndim
                   ...
                   psc(ineighbor)%conss(ixR^S,idir)=psb(igrid)%conss(ixS^S,idir)
                 end do
               end if
             end if
          {end do\}
          ...
          {do ic^DB=1+int((1-i^DB)/2),2-int((1+i^DB)/2)
             ...
             if(ipe_neighbor==mype) then
               ...
               ixR^L=ixR_p_^L(iib^D,n_inc^D);
               call pole_copy(psc(ineighbor)%prim,ixCoG^L,ixR^L,psb(igrid)%prim,ixG^LL,ixS^L,ipole)
               if(stagger_grid) then
                 do idir=1,ndim
                   ixS^L=ixS_p_stg_^L(idir,inc^D);
                   ixR^L=ixR_p_stg_^L(idir,n_inc^D);
                   call pole_copy_stg(psc(ineighbor)%conss,ixR^L,psb(igrid)%conss,ixS^L,idir,ipole)
                 end do
               end if
             end if
          {end do\}
        ...
        if(ipe_neighbor/=mype) then
          ...
          if (ipole==0) then   !! There is no pole 
            ...
            do idir=1,ndim
              ...
              psc(igrid)%conss(ixR^S,idir)=reshape(source=recvbuffer_p(ibuf_recv_p:ibuf_next-1),&
                    shape=shape(psc(igrid)%conss(ixR^S,idir)))
            ...
            do idir=1,ndim
              ixR^L=ixR_p_stg_^L(idir,inc^D);
              ibuf_next=ibuf_recv_p+sizes_p_recv_stg(idir,inc^D)
              pole_buf%conss(ixR^S,idir)=reshape(source=recvbuffer_p(ibuf_recv_p:ibuf_next-1),&
                shape=shape(psc(igrid)%conss(ixR^S,idir)))
              call pole_copy_stg(psc(igrid)%conss,ixR^L,pole_buf%conss,ixR^L,idir,ipole)
              ibuf_recv_p=ibuf_next
            end do
          end if
        end if
      ...
      subroutine pole_copy_stg(wrecv,ixR^L,wsend,ixS^L,idirs,ipole)
        ...
        double precision :: wrecv(ixGs^T,1:nconss), wsend(ixGs^T,1:nconss)
        integer :: iB, iside
        ...
        {case (^D)
           iside=int((i^D+3)/2)
           iB=2*(^D-1)+iside
           select case (typeboundary(Bvec(idirs),iB))
           case ("symm")
