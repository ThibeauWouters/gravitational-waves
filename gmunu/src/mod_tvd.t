...
contains
  ...
  subroutine tvdlimit(method,qdt,ixI^L,ixO^L,idim^LIM,s,qt,snew,fC,dx^D,x)
    ...
    double precision, dimension(ixI^S,ncons) :: cons, cons_new
    type(state) :: s, snew
    double precision, intent(in) :: x(ixI^S,1:ndim)
    double precision :: fC(ixI^S,1:ncons,1:ndim)

    integer :: idims, ixIC^L, jxIC^L
    double precision, dimension(ixI^S,ncons) :: consR, consL

    associate(cons=>s%cons,cons_new=>snew%cons)
    do idims= idim^LIM
       ixICmax^D=ixOmax^D+kr(idims,^D); ixICmin^D=ixOmin^D-2*kr(idims,^D);
       consL(ixIC^S,1:ncons)=cons(ixIC^S,1:ncons)
       jxIC^L=ixIC^L+kr(idims,^D);
       consR(ixIC^S,1:ncons)=cons(jxIC^S,1:ncons)
       call tvdlimit2(method,qdt,ixI^L,ixIC^L,ixO^L,idims,consL,consR,cons_new,x,fC,dx^D)
    end do
    end associate
  end subroutine tvdlimit

  subroutine tvdlimit2(method,qdt,ixI^L,ixIC^L,ixO^L,idims,consL,consR,cons_new,x,fC,dx^D)

    ! Limit the flow variables in cons_new according to typetvd. 
    ! wroeC is based on consL and consR.
    ...
    double precision, dimension(ixG^T,ncons) :: consL, consR
    double precision, intent(in) :: x(ixI^S,1:ndim)
    double precision :: cons_new(ixI^S,1:ncons)
    double precision :: fC(ixI^S,1:ncons,1:ndim)

    double precision:: workroe(ixG^T,1:nworkroe)
    double precision, dimension(ixG^T,ncons) :: wroeC
    ...
    call phys_average(consL,consR,x,ixIC^L,idims,wroeC,workroe)
    ...
    do il=1,ncons
       ...
       call phys_get_eigenjump(consL,consR,wroeC,x,ixIC^L,il,idims,smallaC,adtdxC,jumpC,workroe)
       ...
       do iw=1,ncons
          ...
          if (slab_uniform) then
             ...
             cons_new(ixO^S,iw)=cons_new(ixO^S,iw)+rphiC(ixO^S)-rphiC(hxO^S)
          else
