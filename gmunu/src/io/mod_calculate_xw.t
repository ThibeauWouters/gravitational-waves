...
contains
  ...
  subroutine calc_grid(qunit,igrid,xC,xCC,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,&
    ...
    ! possible normalization values for the nprim+1:nprim+nwauxio entries
  ...
  subroutine to_cartesian(x_TMP,w_TMP,ix^L,xC,wC)
    ...
    {do ix^DB=ixmin^DB,ixmax^DB\}
       ...
       case (cylindrical)
          ...
          do iw=1,nprim+nwauxio
             if (iw<=nprim) iw0=vectoriw(iw)
             if (iw0>=0.and.iw<=iw0+ndim.and.iw<=nprim) then
                idim=iw-iw0
...
!    end if
    ...
    ! then all nprim variables, with w_write control for inclusion
