...
contains
  ...
  !> Read the command line arguments passed to gmunu
  subroutine read_arguments()
    ...
    if (mype == 0) then
       ...
       print *, ' | |                  _____                                               | | '
       print *, ' | |                 /  ___| __  __  _   _  __  _  _   _                  | | '
       print *, ' | |                 | | __ |  \/  || | | ||  \| || | | |                 | | '
       print *, ' | |                 | |_\ || |\/| || |_| || |\\ || |_| |                 | | '
       print *, ' | |                 \_____||_|  |_||_____||_| \_||_____|                 | | '
       print *, ' | |______________________________________________________________________| | '         
       print *, ' |__________________________________________________________________________| '         
    end if
    ...
    if (help) then
       if (mype == 0) then
          print *, 'Usage example:'
          print *, 'mpirun -np 4 ./gmunu -i file.par [file2.par ...]'
  ...
  subroutine read_par_files()
    ...
    namelist /filelist/ base_filename,restart_from_file, &
         ...
         convert,convert_type,usr_filename,&
    ...
    namelist /methodlist/ time_stepper,time_integrator, &
         ...
         loglimit,&
         typetvd,typeentropy,typeaverage, &
         ...
         small_values_method, small_values_daverage, fix_small_values, &
         small_values_fix_iw, &
         check_NaN_values
    ...
    {
    allocate(typeboundary_min^D(nprimbc))
    allocate(typeboundary_max^D(nprimbc))
    typeboundary_min^D = undefined
    typeboundary_max^D = undefined
    }

    allocate(typeboundary(nprim, 2 * ndim))
    ...
    allocate(small_values_fix_iw(nprim))
    ...
    slice_type               = 'vtuCC'
    collapse_type            = 'vti'
    allocate(w_write(nprim))
    w_write(1:nprim)             = .true.
    ...
    allocate(w_convert_factor(nprim))
    ...
    refine_criterion               = 3
    allocate(w_refine_weight(nprim+1))
    w_refine_weight                    = 0.d0
    allocate(logflag(nprim+1))
    logflag                     = .false.
    ...
    qstretch_baselevel(1:ndim) = 1.0d0
    ...
    flatsh          = .false.
    typesourcesplit = 'sfs'
    allocate(loglimit(nprim))
    loglimit(1:nprim)  = .false.

    positivity_preserving  = .false.

    allocate(typeentropy(nprim))
    ...
    ! Initialize Kronecker delta, and Levi-Civita symbols
    do i=1,3
       do j=1,3
          if(i==j)then
             kr(i,j)=1
    ...
    do iw=0,3
       do i=0,3
          do j=0,3
             do k=0,3
                   four_lvc(iw,i,j,k)= sgn(k-j)*sgn(k-i)*sgn(k-iw) &
                                      *sgn(j-i)*sgn(j-iw) &
                                      *sgn(i-iw)
             enddo
          enddo
...
108    close(unitpar)
    ...
    if (ditsave_log < biginteger) ditsave(1) = ditsave_log
    if (ditsave_dat < biginteger) ditsave(2) = ditsave_dat
    if (ditsave_slice < biginteger) ditsave(3) = ditsave_slice
    if (ditsave_collapsed < biginteger) ditsave(4) = ditsave_collapsed
    if (ditsave_custom < biginteger) ditsave(5) = ditsave_custom
    ...
    if (any(typeboundary_min^D /= undefined)) then
      typeboundary(1:nprimbc, 2*^D-1) = typeboundary_min^D(1:nprimbc)
    end if

    if (any(typeboundary_max^D /= undefined)) then
      typeboundary(1:nprimbc, 2*^D) = typeboundary_max^D(1:nprimbc)
    end if
    }

    ! default: without speciciing BC, take the same boundary type as scalar
    if (nprimbc<nprim) then
      do iw = nprimbc+1, nprim
        typeboundary(iw,:) = typeboundary(1, :)
      end do
      ! take the same boundary type as veloc for vectors
      if (nvector > 1) then
        do iw = 2, nvector
          do i = 1, ndir
            if ( iw_vector(iw) > nprimbc ) then 
               typeboundary(iw_vector(iw)+i,:) = typeboundary(iw_vector(1)+i, :)
            end if
          end do
        end do
      end if
  ...
  subroutine snapshot_write_header(fh, offset_tree, offset_block)
    ...
    call MPI_FILE_WRITE(fh, nprim, 1, MPI_INTEGER, st, er)
    ...
    do iw = 1, nprim
      call MPI_FILE_WRITE(fh, prim_names(iw), name_len, MPI_CHARACTER, st, er)
    end do
  ...
  subroutine snapshot_read_header(fh, offset_tree, offset_block)
    ...
    call MPI_FILE_READ(fh, ibuf(1), 1, MPI_INTEGER, st, er)
    nw_found=ibuf(1)
    if (nprim /= ibuf(1)) then
      write(*,*) "nprim=",nprim," and nprim found in restart file=",ibuf(1)
      write(*,*) "Please be aware of changes in w at restart."
      !call mpistop("currently, changing nprim at restart is not allowed")
    end if
  ...
  subroutine write_snapshot
    ...
    n_values = count_ix(ixG^LL) * nprim
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGs^LL) * nconss
    end if
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      if(stagger_grid) then
        w_buffer(1:n_values) = pack(ps(igrid)%prim(ixO^S, 1:nprim), .true.)
        {ixOsmin^D = ixOmin^D -1\}
        {ixOsmax^D = ixOmax^D \}
        n_values_stagger= count_ix(ixOs^L)*nconss
        w_buffer(n_values+1:n_values+n_values_stagger) = pack(ps(igrid)%conss(ixOs^S, 1:nconss), .true.)
      ...
      do ipe = 0, npe-1
        do Morton_no=Morton_start(ipe),Morton_stop(ipe)
          ...
          if(stagger_grid) then
            ...
            n_values = n_values + count_ix(ixOs^L) * nconss
          end if
          ...
          if (mype == ipe) then ! Root task
            ...
            if(stagger_grid) then
              ...
              ps(igrid)%conss(ixOs^S,1:nconss)=reshape(w_buffer(n_values_stagger+1:n_values), &
                   shape(ws(ixOs^S, 1:nconss)))
            else 
              w(ixO^S, 1:nw_found) = reshape(w_buffer(1:n_values), &
                   shape(w(ixO^S, 1:nw_found)))
            end if
            if (nw_found<nprim) then
              if (associated(usr_transform_w)) then
                call usr_transform_w(ixG^LL,ixM^LL,nw_found,w,ps(igrid)%x,ps(igrid)%prim)
              else
                ps(igrid)%prim(ixO^S,1:nw_found)=w(ixO^S,1:nw_found)
              end if
            else if (nw_found>nprim) then
              if (associated(usr_transform_w)) then
                call usr_transform_w(ixG^LL,ixM^LL,nw_found,w,ps(igrid)%x,ps(igrid)%prim)
              else
                ps(igrid)%prim(ixO^S,1:nprim)=w(ixO^S,1:nprim)
              end if
            else
              ps(igrid)%prim(ixO^S,1:nprim)=w(ixO^S,1:nprim)
            end if
  ...
  subroutine read_snapshot_old()
    ...
    double precision              :: wio(ixG^T,1:nprim)
    ...
    if (mype==0) then
      ...
      if (nprim/=nwini) then
        write(*,*) "nprim=",nprim," and nprim in restart file=",nwini
        call mpistop("currently, changing nprim at restart is not allowed")
      end if
      ...
      if (ixGhi^D/=nxini^D+2*nghostcells|.or.) then
        ...
        call mpistop("change with setgmunu")
      end if
    ...
    if (mype==0)then
      ...
      do Morton_no=Morton_start(0),Morton_stop(0)
        ...
        call MPI_FILE_READ_AT(fh,offset,ps(igrid)%prim,1,type_block_io, &
             istatus,ierrmpi)
      end do
      if (npe>1) then
        do ipe=1,npe-1
      ...
      do Morton_no=Morton_start(mype),Morton_stop(mype)
        ...
        itag=Morton_no
        inrecv=inrecv+1
        call MPI_RECV(ps(igrid)%prim,1,type_block_io,0,itag,icomm,&
  ...
  subroutine printlog_default
    ...
    if (mype == 0) then
       ...
       if (.not. opened) then
          ...
          do level=1,nprim
             i = len_trim(line) + 2
             write(line(i:),"(a,a)") trim(cons_names(level)), " "
          end do
  ...
  subroutine printlog_regression_test()
    ...
    integer            :: power
    double precision   :: modes(nprim, n_modes), volume
    ...
    if (mype == 0) then
       ...
       write(fmt_string, "(a,i0,a)") "(", nprim * n_modes + 1, fmt_r // ")"
  ...
  subroutine get_volume_average(power, mode, volume)
    ...
    double precision, intent(out) :: mode(nprim)  !< The computed mode
    double precision, intent(out) :: volume    !< The total grid volume
    integer                       :: iigrid, igrid, iw
    double precision              :: wsum(nprim+1)
    double precision              :: dsum_recv(1:nprim+1)
    ...
    do iigrid = 1, igridstail
       ...
       wsum(nprim+1) = wsum(nprim+1) + sum(ps(igrid)%dvolume(ixM^T))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do iw = 1, nprim
          wsum(iw) = wsum(iw) + &
               sum(ps(igrid)%dvolume(ixM^T)*ps(igrid)%prim(ixM^T,iw)**power)
       end do
    end do

    ! Make the information available on all tasks
    call MPI_ALLREDUCE(wsum, dsum_recv, nprim+1, MPI_DOUBLE_PRECISION, &
         MPI_SUM, icomm, ierrmpi)

    ! Set the volume and the average
    volume = dsum_recv(nprim+1)
    mode   = dsum_recv(1:nprim) / volume
  ...
  subroutine get_global_maxima(wmax)
    ...
    double precision, intent(out) :: wmax(nprim)  !< The global maxima

    integer                       :: iigrid, igrid, iw
    double precision              :: wmax_mype(nprim),wmax_recv(nprim)

    wmax_mype(1:nprim) = -bigdouble

    ! Loop over all the grids
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       do iw = 1, nprim
          wmax_mype(iw)=max(wmax_mype(iw),maxval(ps(igrid)%prim(ixM^T,iw)))
       end do
    end do

    ! Make the information available on all tasks
    call MPI_ALLREDUCE(wmax_mype, wmax_recv, nprim, MPI_DOUBLE_PRECISION, &
         MPI_MAX, icomm, ierrmpi)

    wmax(1:nprim)=wmax_recv(1:nprim)
  ...
  subroutine get_global_minima(wmin)
    ...
    double precision, intent(out) :: wmin(nprim)  !< The global maxima

    integer                       :: iigrid, igrid, iw
    double precision              :: wmin_mype(nprim),wmin_recv(nprim)

    wmin_mype(1:nprim) = bigdouble

    ! Loop over all the grids
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       do iw = 1, nprim
          wmin_mype(iw)=min(wmin_mype(iw),minval(ps(igrid)%prim(ixM^T,iw)))
       end do
    end do

    ! Make the information available on all tasks
    call MPI_ALLREDUCE(wmin_mype, wmin_recv, nprim, MPI_DOUBLE_PRECISION, &
         MPI_MIN, icomm, ierrmpi)

    wmin(1:nprim)=wmin_recv(1:nprim)
