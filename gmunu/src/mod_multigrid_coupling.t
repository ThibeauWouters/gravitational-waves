...
contains
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_from)) then
          mg%boxes(id)%cc({0:nc+1}, iw_to) = &
               fac * state_from(igrid)%prim({ilo^D:ihi^D}, iw_from)
       else
          mg%boxes(id)%cc({0:nc+1}, iw_to) = fac * ps(igrid)%prim({ilo^D:ihi^D}, iw_from)
       end if
    end do
  ...
  subroutine mg_copy_from_tree(iw_from, iw_to, state_to)
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_to)) then
          state_to(igrid)%prim(ixM^T, iw_to) = mg%boxes(id)%cc({1:nc}, iw_from)
       else
          ps(igrid)%prim(ixM^T, iw_to) = mg%boxes(id)%cc({1:nc}, iw_from)
       end if
    end do
  ...
  subroutine mg_copy_from_tree_gc(iw_from, iw_to, state_to)
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_to)) then
          state_to(igrid)%prim({ilo^D:ihi^D}, iw_to) = mg%boxes(id)%cc({0:nc+1}, iw_from)
       else
          ps(igrid)%prim({ilo^D:ihi^D}, iw_to) = mg%boxes(id)%cc({0:nc+1}, iw_from)
       end if
    end do
  end subroutine mg_copy_from_tree_gc

  !> Generate a multigrid tree that includes the gmunu tree, but also contains
  ...
  subroutine mg_tree_from_gmunu(mg)
    use mod_forest
    use mod_global_parameters
    type(mg_t), intent(inout)        :: mg
  ...
  end subroutine mg_tree_from_gmunu
