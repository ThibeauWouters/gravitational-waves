   ...
   subroutine allocateBflux
     ...
     do iigrid=1,igridstail; igrid=igrids(iigrid);
       ...
       {do iside=1,2
         ...
         case(neighbor_fine)
           allocate(pflux(iside,^D,igrid)%flux(1^D%1:nx^DD,1:ncons))
           if(stagger_grid) allocate(pflux(iside,^D,igrid)%edge(1^D%0:nx^DD,1:ndim-1))
         case(neighbor_coarse)
           allocate(pflux(iside,^D,igrid)%flux(1^D%1:nxCo^DD,1:ncons))
       ...
       do idims= idim^LIM
         select case (idims)
           ...
           do iside=1,2
             ...
             ! remove coarse flux
             if (slab_uniform) then
               psb(igrid)%cons(ix^D%ixM^T,nw0:nw1) &
                    = psb(igrid)%cons(ix^D%ixM^T,nw0:nw1) &
                    -pflux(iside,^D,igrid)%flux(1^D%:^DD&,1:nwfluxin)
             else
               do iw=nw0,nw1
                 psb(igrid)%cons(ix^D%ixM^T,iw)=psb(igrid)%cons(ix^D%ixM^T,iw)&
            ...
            {do ic^DDB=1+int((1-i^DDB)/2),2-int((1+i^DDB)/2)
               ...
               if (ipe_neighbor==mype) then
                 iotherside=3-iside
                 if (slab_uniform) then
                   psb(igrid)%cons(ix^S,nw0:nw1) &
                        = psb(igrid)%cons(ix^S,nw0:nw1) &
                        + pflux(iotherside,^D,ineighbor)%flux(:^DD&,1:nwfluxin)&
                        * CoFiratio
                 else
                   do iw=nw0,nw1
                     psb(igrid)%cons(ix^S,iw)=psb(igrid)%cons(ix^S,iw) &
                 ...
                 if (slab_uniform) then
                   ...
                   psb(igrid)%cons(ix^S,nw0:nw1) &
                        = psb(igrid)%cons(ix^S,nw0:nw1)+CoFiratio &
                        *reshape(source=recvbuffer(ibuf:ibufnext-1), &
                        shape=shape(psb(igrid)%cons(ix^S,nw0:nw1)))
                   ibuf=ibuf+isize(^D)
                 else
                   ibufnext=ibuf+isize(^D)
                   if(stagger_grid) then
                   ...
                   do iw=nw0,nw1
                     psb(igrid)%cons(ix^S,iw)=psb(igrid)%cons(ix^S,iw) &
                          +reshape(source=recvbuffer(ibuf:ibufnext-1), &
                          shape=shape(psb(igrid)%cons(ix^S,iw))) &
                          /ps(igrid)%dvolume(ix^S)
                     ibuf=ibuf+nbuf
                   end do
                   ibuf=ibufnext
