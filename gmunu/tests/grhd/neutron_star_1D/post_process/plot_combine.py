...
if (FFT):
  ...
  ax2.plot(df_FD['f'], df_FD['fft'] ,linestyle = '-' ,linewidth = 1.0, label = '$\\rm{FFT}$$(\\rho_{c})$' )
  #df_FD = pd.read_csv("W_vel_FD.csv", delimiter = '\t', usecols = ['f','fft1','psd1'])
  ...
  ax2.set_ylabel('$\\alpha_{\\rm{min}}(t)$')
  #ax2.set_ylabel('$[\\rho_{c}(t)/\\rho_{c}(0) - 1]$')
  ax2.set_xlabel('$t$ $\\rm{(ms)}$')
  x, y = np.loadtxt(filepath+"output.log", unpack=True ,dtype='float',usecols=(1,4),skiprows=1)
  x = x / 2.03001708e05 * 1000 # s to ms
  ax2.plot(x , y, linestyle = '-' ,linewidth = 1.0 )
  ax2.set_yscale('log')
  #ax1.legend(loc='best')
  ax2.grid(True)
