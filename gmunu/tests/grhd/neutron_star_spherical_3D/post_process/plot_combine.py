...
if (FFT):
  #FFT part
  df_FD = pd.read_csv("W_vel_FD.csv", delimiter = '\t', usecols = ['f','fft1','psd1','fft2','psd2','fft3','psd3'])
  df_FD = df_FD[['f','fft1','psd1','fft2','psd2','fft3','psd3']]
  ax2.plot(df_FD['f'], df_FD['fft1'] ,linestyle = '-' ,linewidth = 1.0, label = '$\\rm{FFT}$$(v^{x})$' )
  ax2.plot(df_FD['f'], df_FD['fft2'] ,linestyle = '-' ,linewidth = 1.0, label='$\\rm{FFT}$$(v^{y})$' )
  ax2.plot(df_FD['f'], df_FD['fft3'] ,linestyle = '-' ,linewidth = 1.0, label='$\\rm{FFT}$$(v^{z})$' )
