module mod_usr
  ...
  ! note: if do migration, set initialize_metric = F
  logical, parameter  ::  initialize_metric = .True.

  logical, parameter  ::  exclude_atmo = .True.
...
contains
  ...
  subroutine printlog()
    ...
    double precision     :: star_flag(ixG^T)
    ...
    do iigrid = 1, igridstail
       ...
       ! exclude atmosphere if needed
       star_flag(ixM^T) = 1.0d0
       if (exclude_atmo) then
          where ( ps(igrid)%prim(ixM^T,rho_) <= small_rho_thr )
             star_flag(ixM^T) = 0.0d0
          end where
       end if

       ! calculate total volume
       dV(ixM^T) = ps(igrid)%prim(ixM^T,psi_)**6*ps(igrid)%dvolume(ixM^T)*star_flag(ixM^T)
       ...
       send_buffer(total_mass) = send_buffer(total_mass) + &
          sum( ps(igrid)%cons(ixM^T,D_) * ps(igrid)%dvolume(ixM^T)*star_flag(ixM^T) )
