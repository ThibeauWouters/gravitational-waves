module mod_usr
  ...
  double precision, parameter            :: resistivity = 1.0d-6
...
contains
  ...
  subroutine printlog()
    ...
    integer, parameter   :: total_divEdV = 4
    integer, parameter   :: total_divE2dV = 5
    integer, parameter   :: total_volume = 6
    ...
    do iigrid = 1, igridstail
       ...
       send_buffer(total_mass) = send_buffer(total_mass) + &
          sum(lfac(ixM^T) * ps(igrid)%prim(ixM^T,rho_) * ps(igrid)%dvolume(ixM^T))
       ...
       send_buffer(total_divEdV) = send_buffer(total_divEdV) + &
          sum(dabs(ps(igrid)%prim(ixM^T,divE_)) * ps(igrid)%dvolume(ixM^T))

       ! calculate total divE^2 dV
       send_buffer(total_divE2dV) = send_buffer(total_divE2dV) + &
          sum(ps(igrid)%prim(ixM^T,divE_)**2 * ps(igrid)%dvolume(ixM^T))
    end do
    ...
    ! L2 norm of divE
    recive_buffer(total_divE2dV) = dsqrt( recive_buffer(total_divE2dV) / recive_buffer(total_volume) )
    ...
    if (mype == 0) then
       ...
       if (.not. opened) then
          ...
          line = "it global_time dt M0 avg_divB L2_divB avg_divE L2_divE"
