module mod_usr
  ...
  double precision, parameter            :: zp = 1.0d0
  double precision, parameter            :: zm = 0.5d0

  double precision, parameter            :: X_0 = -1.0d0
  double precision, parameter            :: Y_0 = -0.0d0
...
contains
  ...
  subroutine rm_init_one_grid(ixI^L,ixO^L,w,x)
    ...
    double precision                :: equ, cart_z, cylindrical_rho
    ...
    ! in cart coord.
    v(ixO^S, 1)   =  dsqrt( 0.05d0 )
    lfac(ixO^S) =  0.05d0 
    lfac(ixO^S) = 1.0d0 / dsqrt( 1.0d0 - lfac(ixO^S) )

    ! transform in spherical coordinate
    w(ixO^S,W_vel(1))   =   v(ixO^S, 1) * dsin( x(ixO^S,2) ) * dcos( x(ixO^S,3) ) 
    w(ixO^S,W_vel(2))   =   v(ixO^S, 1) * dcos( x(ixO^S,2) ) * dcos( x(ixO^S,3) ) 
    w(ixO^S,W_vel(3))   = - v(ixO^S, 1) * dsin( x(ixO^S,3) ) 

    w(ixO^S,W_vel(1))   = w(ixO^S,W_vel(1)) * lfac(ixO^S)
    w(ixO^S,W_vel(2))   = w(ixO^S,W_vel(2)) * lfac(ixO^S) / x(ixO^S,1)
    w(ixO^S,W_vel(3))   = w(ixO^S,W_vel(3)) * lfac(ixO^S) / x(ixO^S,1) / dsin( x(ixO^S,2) )

    {do ix^D = ixO^LIM^D \}
       cart_z = x(ix^D,1) * dcos( x(ix^D,2) )
       cylindrical_rho = x(ix^D,1) * dsin( x(ix^D,2) )
       equ = (cylindrical_rho)**2 - 2 * cylindrical_rho * ( X_0 * dcos(x(ix^D,3)) + Y_0 * dsin(x(ix^D,3)) )&
             - r_loop**2 + X_0**2 + Y_0**2
       if ( equ <= 0.0d0 .and. cart_z <= zp .and. cart_z >= zm ) then
          equ = (cylindrical_rho)**2 - 2 * cylindrical_rho * ( X_0 * dcos(x(ix^D,3)) + Y_0 * dsin(x(ix^D,3)) ) + X_0**2 + Y_0**2
          equ = dsqrt( dabs(equ) )
          Bx = - A_loop / equ * ( cylindrical_rho * dsin(x(ix^D,3)) - Y_0 )
          By =   A_loop / equ * ( cylindrical_rho * dcos(x(ix^D,3)) - X_0 )
          w(ix^D,Bvec(1))    =  Bx * dsin(x(ix^D,2)) * dcos(x(ix^D,3)) + By * dsin(x(ix^D,2)) * dsin(x(ix^D,3)) 
          w(ix^D,Bvec(2))    =  Bx * dcos(x(ix^D,2)) * dcos(x(ix^D,3)) + By * dcos(x(ix^D,2)) * dsin(x(ix^D,3)) 
          w(ix^D,Bvec(3))    = -Bx * dsin(x(ix^D,3)) + By * dcos(x(ix^D,3)) 

          w(ix^D,Bvec(2))    =  w(ix^D,Bvec(2)) / x(ix^D,1)
          w(ix^D,Bvec(3))    =  w(ix^D,Bvec(3)) / x(ix^D,1) / dsin(x(ix^D,2))
       end if
    {enddo^D&\}
